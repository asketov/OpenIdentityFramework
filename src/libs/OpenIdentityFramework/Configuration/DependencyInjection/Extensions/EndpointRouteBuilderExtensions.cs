        ...
        where THandler : class, IEndpointHandler
    {
        ...
        var endpointBuilder = builder.Map(
            ...
            static async httpContext =>
            {
                ...
                var result = await handler.HandleAsync(httpContext, httpContext.RequestAborted).ConfigureAwait(false);
                await result.ExecuteAsync(httpContext, httpContext.RequestAborted).ConfigureAwait(false);
            });
